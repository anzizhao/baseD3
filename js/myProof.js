...
function timeline(domElement) {
    ...
    timeline.xAxis = function (bandName, orientation) {
        ...
        xAxis.redraw = function () {
            //console.log('modeYear', modeYear? "true":"false")
            xAxis.call(axis);
            // navi axis 才能放大缩小
            // 这里就写死罗 TODO
            if( bandName === 'naviBand' ) {
                // TODO maybe 不需timeline.redrawd, 其实可以具体的
                // 因为的brush.redraw 会导致的band重新redraw
                xAxis.call(
                    d3.behavior.zoom()
                    .x( band.xScale )
                    .scaleExtent( [0, 10])
        ...
        xAxis.tickFormatToMonth = function () {
            console.log('tickFormatToMonth')
            axis
        ...
        xAxis.tickFormatToCenter = function () {
            console.log('tickFormatToCenter')
            axis  
            .ticks(10)
            .tickFormat(
                function (d) { return toYear(d); }
        ...
        band.xAxis = xAxis      // for chang x axis 
    ...
    timeline.brush = function (bandName, targetNames) {
        ...
        xBrush.redraw = function () {
            xBrush.call(brush);
            onBrush()
        };
        components.push( xBrush )
    ...
    // Utility functions
    //
    timeline.changeAxisTickFormat = function (){
        switch(mode) {
            case 'y': 
                break
            case 'm': 
